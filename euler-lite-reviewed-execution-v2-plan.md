...
After acceptance, the generic `executeTransactionPlan` and `executePreparedTransactionPlan` paths are unreachable. The EOA adapter passes either the reviewed static pre-Pyth prefix or the already-finalized exact suffix/vector to `executeMaterialized`. Its awaited hooks reassert Lite's active invocation guard, wallet binding, policy, and byte equality before every wallet prompt. The first JIT Pyth-bearing suffix request additionally revalidates wallet and policy after refresh, then synchronously checks the refreshed publish-time evidence after the final awaited guard. The SDK does not run planners, plugins, approval resolution, or calldata composition on this path. The Safe adapter submits the complete finalized envelope derived from the same SDK vector through one `wallet_sendCalls` request.
...
- For a typed migration authorization, the exact plan sealed for review retains the SDK's placeholder signature call so the declared signature slot can be verified and filled. State simulation uses the SDK's matching simulation plan, which omits that placeholder call and models the granted authorization with the SDK-provided state override. The simulation plan is never review or execution authority and cannot replace any sealed request byte.
...
Migration revocation remains part of normal execution, but it is not a durable safety gate. A conclusive abort after a confirmed temporary grant dispatches the exact paired reviewed revocation. Unknown core status never triggers cleanup because the core may still execute. If the migration batch or its revocation fails, is rejected, or has unknown status, Lite reports exactly which phase is affected and whether authorization may remain. It releases local submission state and never blocks a fresh migration on completing an old revocation.
...
A completion from context A must never clear or redirect context B. Successful submission updates only the exact stable intent IDs and revisions captured by the reviewed execution; newer cart edits survive. These authoritative completion effects, including external-migration refresh scheduling, run even when a detached Safe suppresses modal, navigation, and visible in-flow success effects. Later confirmation/revert updates are informational and context-scoped. Unknown status does not claim success and does not clear a newer context.
...
10. Resolve approvals and every signature request, including Permit2 nonce and typed data, before review. Locate migration-signature placeholders against the final plan after static plugins have been applied; never reuse indexes discovered in the earlier migration preview.
...
- If the core request receives a successful receipt, continue with its reviewed revocation where applicable. If a confirmed temporary grant is followed by a conclusive core rejection, revert, or pre-core abort, dispatch only the revocation carrying that grant's sealed authorization identity. If core receipt/submission status is unknown, stop without cleanup and report that the authorization may remain.
...
No post-acceptance check may rebuild or mutate the accepted artifact. A failed integrity, wallet-binding, policy, deadline, finalization, or byte-equality check invalidates that reviewed execution and requires fresh preparation and review. If the wallet conclusively rejects the request before any transaction or prerequisite is confirmed, the unchanged reviewed execution may remain available for an explicit second click.
...
- Invalidate the reviewed execution after submission, an unknown outcome, a confirmed prerequisite, or any non-cancellation failure. A conclusive wallet rejection before any request is confirmed, or a conclusive off-chain Safe cancellation, may leave the unchanged reviewed execution available for an explicit second click. This is not an automatic retry and does not authorize rebuilding or changing any request.
