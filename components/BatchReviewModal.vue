...
const isConfirmDisabled = computed(() =>
  isSpyMode.value
  || isExecuting.value
  || hasPendingDetachedExecution.value
  || isPreparing.value
  || isSimulating.value
...
const blockedReason = computed(() => {
  ...
  if (hasPendingDetachedExecution.value) return 'A Safe execution is still pending'
...
const handleExecute = async () => {
  ...
  const assertPreparedPlan = (prepared: TransactionPlanPrepared) => {
  ...
  if (!handle) return
  const run = executeBatch({ scope: handle.scope, assertPreparedPlan })
