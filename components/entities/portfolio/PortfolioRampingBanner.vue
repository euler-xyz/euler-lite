...
import { TTL_INFINITY, TTL_MORE_THAN_ONE_YEAR } from '~/entities/constants'
import { getBorrowPositionTimeToLiquidation } from '~/utils/ltv'
...
const statuses = computed(() =>
  borrowPositions.value.map((position) => {
    const ttl = getBorrowPositionTimeToLiquidation(position)
    return {
      isRamping: ttl !== TTL_INFINITY,
      willBeLiquidated: ttl !== TTL_INFINITY && ttl !== TTL_MORE_THAN_ONE_YEAR,
    }
  }),
)
