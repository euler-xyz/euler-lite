...
const { isReviewWalletContextInvalidated } = useWalletReviewContext({
  account: walletAddress,
  chainId: currentChainId,
  isSubmitting: internalSubmitting,
  onInvalidated: () => emits('close'),
})
...
const isConfirmDisabled = computed(() => isSpyMode.value || isReviewWalletContextInvalidated.value || internalSubmitting.value || hasPendingDetachedExecution.value || isPreparingPlan.value || isResolvingStateOverrideHints.value || !!prepareError.value || (!reviewPlan.value?.length && !hasDisplayOnlyConfirmation.value))
