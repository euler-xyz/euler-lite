...
import { makeCeremony } from './fixtures'

describe('durable ceremony journal contract', () => {
  it('atomically fences a wallet lane and rejects stale compare-and-swap writes', async () => {
    ...
    await journal.putCeremony(ceremony)
    const laneKey = walletLaneKey(ceremony.template.wallet.account, ceremony.template.wallet.chainId)
    const attempt = await journal.reserveAttempt({ ceremony, attemptId: 'attempt-1', reservationId: 'reservation-1', laneKey, requestVectorDigest: requestVectorDigest(ceremony), now: 10 })

    await expect(journal.reserveAttempt({ ceremony, attemptId: 'attempt-2', reservationId: 'reservation-2', laneKey, requestVectorDigest: requestVectorDigest(ceremony), now: 11 }))
  ...
  it('reads back every reservation field and keeps external identifiers monotonic', async () => {
    ...
    await journal.putCeremony(ceremony)
    const laneKey = walletLaneKey(ceremony.template.wallet.account, ceremony.template.wallet.chainId)
    let attempt = await journal.reserveAttempt({ ceremony, attemptId: 'attempt-1', reservationId: 'reservation-1', laneKey, requestVectorDigest: requestVectorDigest(ceremony), now: 10 })
    await expect(journal.verifyReservation({
      attemptId: attempt.attemptId, reservationId: 'wrong', ceremonyId: ceremony.ceremonyId,
      templateDigest: ceremony.templateDigest, account: attempt.account, chainId: attempt.chainId,
      laneKey, requestVectorDigest: requestVectorDigest(ceremony), version: attempt.version, fence: attempt.fence,
  ...
  it('never releases ambiguous or otherwise non-terminal attempts', async () => {
    ...
    const attempt = await journal.reserveAttempt({
      ceremony, attemptId: 'attempt-1', reservationId: 'reservation-1',
      laneKey: walletLaneKey(ceremony.template.wallet.account, ceremony.template.wallet.chainId),
      requestVectorDigest: requestVectorDigest(ceremony), now: 10,
    })
