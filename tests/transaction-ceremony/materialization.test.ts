...
import { validateExecutionTemplate } from '~/features/transaction-ceremony/domain/validators'
import { executionTemplateDigest, materializePreparedPlan, normalizedRequestDigest } from '~/features/transaction-ceremony/materialization/prepared-plan'
import { assertPermit2NonceCurrent, permit2NonceCoordinate, type PreparedPermit2Slot } from '~/features/transaction-ceremony/materialization/signature-slots'
...
describe('prepared plan materialization', () => {
  it('materializes a deterministic EOA request vector and complete effect graph', () => {
    ...
    const first = materializePreparedPlan({ intents: [intent], plan, wallet: eoa, sdk, policyEvidenceDigest: POLICY })
    const second = materializePreparedPlan({ intents: [intent], plan, wallet: eoa, sdk, policyEvidenceDigest: POLICY })

    expect(executionTemplateDigest(first)).toBe(executionTemplateDigest(second))
  ...
  it('consumes and verifies the SDK-owned deterministic request composition', () => {
    ...
    expect(() => materializePreparedPlan({ intents: [intent], plan, wallet: eoa, sdk: verifyingSdk, policyEvidenceDigest: POLICY })).not.toThrow()
    expect(calls).toBe(1)
    const driftingSdk = {
      ...verifyingSdk,
      executionService: {
    ...
    expect(() => materializePreparedPlan({ intents: [intent], plan, wallet: eoa, sdk: driftingSdk, policyEvidenceDigest: POLICY }))
  ...
  it('resolves transport before sealing and never reclassifies the call vector', () => {
    ...
    const template = materializePreparedPlan({ intents: [intent], plan, wallet: safeWallet, sdk, policyEvidenceDigest: POLICY })
    expect(template.transport).toBe('safe')
    expect(template.requests).toHaveLength(1)
    expect(template.requests[0]).toHaveProperty('callId')
    expect(template.requests[0]).not.toHaveProperty('requestId')
  })
  ...
  it('binds Permit2 typed data to an ABI-aware insertion point', async () => {
    ...
    const template = materializePreparedPlan({
      intents: [intent],
      plan,
      wallet: { ...eoa, approvalMode: 'permit2' },
      sdk,
      permit2Slots: [permitSlot],
      policyEvidenceDigest: POLICY,
    })

    expect(template.signatureSlots).toHaveLength(1)
    expect(template.signatureSlots[0].typedDataHash).toBe(permitSlot.typedDataHash)
    expect(template.signatureSlots[0].validUntil).toBe(2_000_000_000)
    expect(template.signatureSlots[0].insertionPoints).toEqual([expect.objectContaining({ batchItemIndex: 0, abiArgumentPath: ['signature'] })])
    expect(template.effects[0].effect).toMatchObject({ kind: 'approval', mode: 'permit2', owner: ACCOUNT, token: TOKEN, spender: SPENDER, amount: 10n })
    expect(() => validateExecutionTemplate(template, [intent])).not.toThrow()
    expect(() => validateExecutionTemplate({
      ...template,
      signatureSlots: [{ ...template.signatureSlots[0], typedDataHash: keccak256(toHex('tampered')) }],
    }, [intent])).toThrow(/typed-data digest changed/)
    expect(() => validateExecutionTemplate({
      ...template,
      signatureSlots: [{
        ...template.signatureSlots[0],
        insertionPoints: [{ ...template.signatureSlots[0].insertionPoints[0], effectId: template.effects[1].effectId }],
      }],
    }, [intent])).toThrow(/different effect/)
    expect(permit2NonceCoordinate(template.signatureSlots[0])).toEqual({ owner: ACCOUNT, token: TOKEN, spender: SPENDER, permit2: SPENDER, nonce: 7n })
    expect(() => permit2NonceCoordinate({ ...template.signatureSlots[0], nonce: 8n })).toThrow(/nonce does not match/)
    await expect(assertPermit2NonceCurrent(template.signatureSlots[0], async () => 8n)).rejects.toThrow(/nonce changed after review/)
  })
  ...
  it('seals a bounded Pyth slot and rejects missing feed evidence', () => {
    ...
    const template = materializePreparedPlan({ intents: [intent], plan, wallet: eoa, sdk, pythEvidence: evidence, policyEvidenceDigest: POLICY })
    expect(template.pythRefreshSlots).toHaveLength(1)
    expect(template.pythRefreshSlots[0]).toMatchObject({ target: PYTH, previewValue: 2n, maxValue: 10n, previewPublishTimes: [100] })
    expect(() => validateExecutionTemplate(template, [intent])).not.toThrow()
    expect(() => validateExecutionTemplate({
      ...template,
      pythRefreshSlots: [{ ...template.pythRefreshSlots[0], previewPayloadHash: keccak256(toHex('tampered')) }],
    }, [intent])).toThrow(/preview payload digest changed/)
    expect(() => materializePreparedPlan({ intents: [intent], plan, wallet: eoa, sdk, policyEvidenceDigest: POLICY }))
  ...
  it('unwinds cleanup calls in reverse order', () => {
    ...
    expect(template.requests.slice(-2).map(request => request.to)).toEqual([SPENDER, TOKEN])
    expect(template.requests.slice(-2).map(request => request.phase)).toEqual(['cleanup', 'cleanup'])
  })
})
