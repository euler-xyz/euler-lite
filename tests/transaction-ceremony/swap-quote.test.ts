...
import { normalizeCeremonySwapQuote, rehydrateCeremonySwapQuote } from '~/features/transaction-ceremony/domain/swap-quote'
...
describe('ceremony swap quote DTO', () => {
  it('round-trips the public SDK quote through an integer-only exact schema', () => {
    const normalized = normalizeCeremonySwapQuote(quote())
    expect(normalized.slippageBps).toBe(50)
    expect(normalized.tokenIn).not.toHaveProperty('meta')
    expect(rehydrateCeremonySwapQuote(normalized)).toMatchObject({ amountOutMin: '98', slippage: 0.5 })
  ...
  it('rejects slippage that cannot be sealed exactly as basis points', () => {
    expect(() => normalizeCeremonySwapQuote({ ...quote(), slippage: 0.123 } as SwapQuote)).toThrow(/basis points/)
  })
})
