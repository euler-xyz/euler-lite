...
import { assertCanonicalValue, canonicalDigest, deepFreezeCanonical } from '~/features/transaction-ceremony/domain/canonical'
import { assertOperationIntent } from '~/features/transaction-ceremony/domain/schemas'

const ACCOUNT = getAddress('0x1000000000000000000000000000000000000000')

describe('canonical ceremony encoding', () => {
  it('is deterministic across object insertion order and address casing', () => {
    ...
    expect(canonicalDigest('execution-template-v1', first))
      .toBe(canonicalDigest('execution-template-v1', second))
  })

  it('commits to the schema and array order', () => {
    const value = { calls: ['0x01', '0x02'] }
    expect(canonicalDigest('execution-template-v1', value))
      .not.toBe(canonicalDigest('execution-template-v2', value))
    expect(canonicalDigest('execution-template-v1', value))
      .not.toBe(canonicalDigest('execution-template-v1', { calls: ['0x02', '0x01'] }))
  })
