...
const vault = (hookedOps: bigint, hookTarget: string = zeroAddress): Vault =>
  ({ hookedOps, hookTarget } as Vault)

describe('isHookDisabling', () => {
  it('is true when hookTarget is the zero address', () => {
    expect(isHookDisabling(vault(0n, zeroAddress))).toBe(true)
  })
  it('is false when hookTarget is non-zero', () => {
    expect(isHookDisabling(vault(0n, HOOK_TARGET))).toBe(false)
  })
})

describe('isOpHooked', () => {
  it('is true when the bit is set in hookedOps', () => {
    expect(isOpHooked(vault(OP_DEPOSIT | OP_BORROW), OP_DEPOSIT)).toBe(true)
    expect(isOpHooked(vault(OP_DEPOSIT | OP_BORROW), OP_BORROW)).toBe(true)
  })
  it('is false when the bit is not set', () => {
    expect(isOpHooked(vault(OP_DEPOSIT), OP_BORROW)).toBe(false)
    expect(isOpHooked(vault(0n), OP_DEPOSIT)).toBe(false)
  })
})
...
describe('isOpDisabled', () => {
  ...
  it('is true when hookTarget is zero and the specific op bit is set', () => {
    expect(isOpDisabled(vault(OP_DEPOSIT, zeroAddress), OP_DEPOSIT)).toBe(true)
  })
  it('is false when hookTarget is zero but the specific op bit is not set', () => {
    expect(isOpDisabled(vault(OP_BORROW, zeroAddress), OP_DEPOSIT)).toBe(false)
  })
  it('short-circuits to true for any op when OP_VAULT_STATUS_CHECK is disabled', () => {
    // Only the vault-status-check bit is set; deposit is not — but the batch
    // will revert at the end because checkVaultStatus → zero address reverts.
  ...
  it('does not short-circuit when vault-status-check is hooked with a non-zero target', () => {
...
describe('areAllUserOpsHooked', () => {
  it('is true when every user-facing op bit is set', () => {
    const all = OP_DEPOSIT | OP_MINT | OP_WITHDRAW | OP_REDEEM | OP_TRANSFER
      | OP_SKIM | OP_BORROW | OP_REPAY | (1n << 8n) // OP_REPAY_WITH_SHARES
      | (1n << 9n) // OP_PULL_DEBT
      | OP_LIQUIDATE | OP_FLASHLOAN
    expect(areAllUserOpsHooked(all)).toBe(true)
  })
  it('is false when only a subset is set', () => {
    expect(areAllUserOpsHooked(OP_DEPOSIT | OP_BORROW)).toBe(false)
  })
  it('ignores internal ops (OP_VAULT_STATUS_CHECK, OP_TOUCH, OP_CONVERT_FEES)', () => {
    // All internal ops set but no user ops → not "all user ops hooked".
    expect(areAllUserOpsHooked(OP_VAULT_STATUS_CHECK)).toBe(false)
  })
})

describe('isVaultEffectivelyPaused', () => {
  it('is false when the hook target is non-zero, regardless of bits', () => {
    expect(isVaultEffectivelyPaused(vault(OP_VAULT_STATUS_CHECK, HOOK_TARGET))).toBe(false)
  })
  it('is true when OP_VAULT_STATUS_CHECK is disabled (hookTarget zero)', () => {
    expect(isVaultEffectivelyPaused(vault(OP_VAULT_STATUS_CHECK, zeroAddress))).toBe(true)
  })
  it('is true when every user-facing op is disabled even without vault-status-check', () => {
    const allUser = OP_DEPOSIT | OP_MINT | OP_WITHDRAW | OP_REDEEM | OP_TRANSFER
      | OP_SKIM | OP_BORROW | OP_REPAY | (1n << 8n) | (1n << 9n) | OP_LIQUIDATE | OP_FLASHLOAN
...
describe('decodeHookedOps', () => {
  it('returns an empty array for 0n', () => {
    expect(decodeHookedOps(0n)).toEqual([])
  })
  it('returns metadata entries for each set user-facing bit', () => {
    const decoded = decodeHookedOps(OP_DEPOSIT | OP_BORROW)
    expect(decoded.map(op => op.name)).toEqual(['Deposit', 'Borrow'])
  })
  it('hides internal ops by default', () => {
    const decoded = decodeHookedOps(OP_DEPOSIT | OP_VAULT_STATUS_CHECK)
    expect(decoded.map(op => op.name)).toEqual(['Deposit'])
  })
  it('includes internal ops when includeInternal=true', () => {
    const decoded = decodeHookedOps(OP_VAULT_STATUS_CHECK, { includeInternal: true })
...
describe('getOpMeta', () => {
  it('returns the metadata row for a known bit', () => {
    expect(getOpMeta(OP_DEPOSIT)?.name).toBe('Deposit')
    expect(getOpMeta(OP_BORROW)?.name).toBe('Borrow')
  })
  it('returns undefined for an unknown bit', () => {
    expect(getOpMeta(1n << 20n)).toBeUndefined()
  })
})
...
describe('findBlockingDisabledOp', () => {
  ...
  it('identifies blocks coming from the vault-status-check short-circuit', () => {
