...
import { refreshLabelFile } from '~/server/utils/labels-source'

vi.mock('~/server/utils/fetchWithTimeout', () => ({ fetchWithTimeout: vi.fn() }))

describe('label policy source failure handling', () => {
  it('returns an error for a transient failure without a usable cache', async () => {
    ...
    await expect(refreshLabelFile(9001, 'earn-vaults.json')).rejects.toThrow('effective-policy source returned 503')
  })
