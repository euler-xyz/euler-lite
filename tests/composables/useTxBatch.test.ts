...
import { awaitFinalPlanningLayer, buildOperationEntryMap, buildWalletBalanceLayers, buildWalletChanges, countPlanOperations, fetchBaseAccountSnapshot, isBatchEntryGeoBlocked, normalizeSimulatedVaultLayers, stitchAccount, useTxBatch } from '~/composables/useTxBatch'
import {
...
import { collectUnverifiedVaultConsentTargets } from '~/utils/unverified-vault-consent'
...
describe('useTxBatch execution errors', () => {
  ...
  it('does not execute when the final rebuilt plan changes the reviewed vault set', async () => {
    const sdk = createMockSdk()
    vi.mocked(getEulerSdkFresh).mockResolvedValue(sdk as never)
    const batch = useTxBatch()
    const reviewedVault = vault
    const finalVault = aToken
    const planForVault = (targetContract: Address): TransactionPlan => [{
      type: 'evcBatch',
      items: [{ targetContract, onBehalfOfAccount: owner, value: 0n, data: '0x' }],
    }] as TransactionPlan
    const reviewedPlan = planForVault(reviewedVault)
    const finalPlan = planForVault(finalVault)
    eulerTxMocks.estimateGasForPlan.mockResolvedValue(undefined)
    eulerTxMocks.prepareTransactionPlan.mockResolvedValue({
      plan: finalPlan,
      chainId: 1,
      account: owner,
    })
    ...
    await batch.addEntry({
      label: 'Dynamic vault action',
      buildPlan: async () => reviewedPlan,
      buildExecutionPlan: async () => finalPlan,
    })
    await new Promise(resolve => setTimeout(resolve, 0))

    const reviewedTargets = collectUnverifiedVaultConsentTargets(
      [reviewedPlan],
    ...
    await batch.executeBatch({
      assertPreparedPlan: (prepared) => {
        const finalTargets = collectUnverifiedVaultConsentTargets(
          [prepared.plan],
          () => ({ asset: { symbol: 'UNKNOWN' } }),
          () => false,
        ).map(target => target.address)
        if (JSON.stringify(finalTargets) !== JSON.stringify(reviewedTargets)) {
          throw new Error('The final transaction targets a different vault set')
        }
      },
    })
...
describe('useTxBatch execution prerequisites', () => {
  ...
  it('asserts the final prepared safe bundle before submission', async () => {
    const sdk = createMockSdk()
    vi.mocked(getEulerSdkFresh).mockResolvedValue(sdk as never)
    isSafeWalletRef.value = true
    const prepared = { kind: 'prepared' }
    eulerTxMocks.prepareTransactionPlan.mockResolvedValue(prepared)
    const assertPreparedPlan = vi.fn(() => {
      throw new Error('The final transaction targets a different vault set')
    })
    ...
    await addBundledMigrationEntry(batch)
    await batch.prepareBundledExecution()
    await batch.executeBatch({ assertPreparedPlan })

    expect(assertPreparedPlan).toHaveBeenCalledWith(prepared)
