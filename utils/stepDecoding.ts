import { decodeAbiParameters, decodeFunctionData, formatUnits, getAddress, maxUint160, maxUint256, parseAbi, toFunctionSelector, zeroAddress, type Address, type Hex } from 'viem'
  ...
  for (const item of plan) {
    if (item.type === 'requiredApproval') {
      ...
      for (const r of resolved) {
        ...
        const approvalAmount = r.amount === maxUint256 || (r.type === 'permit2' && r.amount === maxUint160)
          ? 'Unlimited'
          : undefined
