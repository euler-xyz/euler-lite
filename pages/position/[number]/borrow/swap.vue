    ...
    modal.open(OperationReviewModal, {
      props: {
        ...
        onConfirm: async (execution, reviewed) => {
          await sendInboundExternalMigration(execution, preview, reviewed)
        },
  ...
  try {
    ...
    try {
      if (preview.bundledReview) {
        ...
        const outcome = await sendInboundExternalMigrationAsSafeBundle(
          input,
          authorizationRequest,
          account,
          useSignatures,
...
const sendInboundExternalMigrationAsSafeBundle = async (
  ...
  useSignatures: boolean,
  reviewedExecution: TransactionPlanPrepared,
  reviewedAuthorizationRequest: MigrationAuthorizationRequest | undefined,
  ...
  const candidatePrepared = await prepareTransactionPlan(simulation.plan, {
    account,
    chainId: input.position.chainId,
    usePermit2: false,
  })
  const prepared = requireReviewedBatchPreparedExecution(reviewedExecution, candidatePrepared, {
    placeholderSignatureCalls: useSignatures
      ? collectReviewedSignaturePlaceholderCalls(reviewedExecution.plan, reviewedAuthorizationRequest)
      : [],
  })
