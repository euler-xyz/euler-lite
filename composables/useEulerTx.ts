...
export const useEulerTx = () => {
  ...
  const executePreparedPlan = async (
    ...
    options?: {
      ...
      /** Must durably arm terminal ambiguity before the Safe receives the terminal EVC request. */
      onSafeTerminalSubmissionStart?: () => void
    ...
    const runPreparedPlan = () => sdk.executionService.executePreparedTransactionPlan({
      ...
      onProgress: (progress) => {
        if (safeWalletProvider && progress.status === 'evcBatch' && !terminalSafeSubmissionArmed) {
          options?.onSafeTerminalSubmissionStart?.()
          terminalSafeSubmissionArmed = true
        }
