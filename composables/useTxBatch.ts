...
export interface ExecuteBatchOptions {
  /** Final review-owned assertion, run on the exact prepared envelope before signing. */
  assertPreparedPlan?: (prepared: TransactionPlanPrepared) => void
  /** Detached execution scope that owns success and post-transaction UI behavior. */
...
export const useTxBatch = () => {
   ...
   */
  const executeBatch = async (options: ExecuteBatchOptions = {}) => {
    ...
    try {
      ...
      if (latchedBundledExecution.value) {
        ...
        await redirectAfterBatchExecution(options.scope)
        return
      }
