...
import type { EoaRequest, FinalizedArtifact } from '../domain/template'
import { AttemptRevertedError, DispatchStatusUnknownError, ProvenPreDispatchCancellationError } from '../coordinator/errors'
import type { CeremonyTransportAdapter, DispatchCallbacks, DispatchResult } from './types'
...
export class EoaCeremonyAdapter implements CeremonyTransportAdapter {
  ...
  async dispatch(ceremony: SealedCeremony, artifact: FinalizedArtifact, callbacks: DispatchCallbacks): Promise<DispatchResult> {
    ...
    const finalized: FinalizedMaterializedExecution = Object.freeze({
      ...
      from: getAddress(ceremony.template.wallet.account),
      evcAddress,
      ...
      const result = await this.executeMaterialized(finalized, {
        ...
        onBeforeStep: async (sdkRequest, stepIndex) => {
          ...
          if (!request || eoaRequestSemanticDigest(sdkRequest) !== eoaRequestSemanticDigest(request)) {
            throw new Error('SDK dispatch request does not match the finalized ceremony')
          }
        ...
        sendTransaction: async (sdkRequest) => {
          ...
          if (!request || eoaRequestSemanticDigest(sdkRequest) !== eoaRequestSemanticDigest(request)) {
            throw new Error('SDK wallet request does not match the finalized ceremony')
          }
          try {
