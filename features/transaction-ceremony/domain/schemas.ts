...
import type { SealedCeremony } from './ceremony'
...
const assertCeremonySwapQuote = (value: unknown, path: string) => {
  assertRecord(value, path)
...
const assertPlannerArgs = (planner: Record<string, unknown>) => {
  ...
  for (const [key, candidate] of Object.entries(planner.args)) {
    if (key === 'swapQuote' || key === 'collateralSwapQuote' || key === 'debtSwapQuote') assertCeremonySwapQuote(candidate, `intent.planner.args.${key}`)
...
const assertEvidenceState = (value: unknown, path: string) => {
  assertRecord(value, path)
...
const assertPolicyEvidence = (value: unknown, path: string) => {
  assertRecord(value, path)
  assertExactKeys(value, ['schemaVersion', 'subjects', 'evidence', 'digest'], path)
  if (value.schemaVersion !== 1) throw new Error(`${path}.schemaVersion is unsupported`)
  if (!Array.isArray(value.subjects) || !Array.isArray(value.evidence)) throw new Error(`${path} arrays are invalid`)
  ...
  value.evidence.forEach((item, index) => {
    ...
    assertRecord(item, evidencePath)
    assertExactKeys(item, ['subject', 'concern', 'result'], evidencePath)
    assertString(item.subject, `${evidencePath}.subject`)
    assertString(item.concern, `${evidencePath}.concern`)
    assertEvidenceState(item.result, `${evidencePath}.result`)
  })
...
const assertSimulation = (value: unknown, path: string) => {
  assertRecord(value, path)
  assertExactKeys(value, ['schemaVersion', 'templateDigest', 'observedAt', 'blockNumber', 'canExecute', 'effects', 'simulatedAccounts', 'simulatedVaults'], path)
  if (value.schemaVersion !== 1) throw new Error(`${path}.schemaVersion is unsupported`)
  assertHash(value.templateDigest, `${path}.templateDigest`)
  assertSafeInteger(value.observedAt, `${path}.observedAt`)
...
const assertManifest = (value: unknown, path: string) => {
  assertRecord(value, path)
  assertExactKeys(value, ['schemaVersion', 'templateDigest', 'entries', 'previewPayloadHashes'], path)
  if (value.schemaVersion !== 1) throw new Error(`${path}.schemaVersion is unsupported`)
  assertHash(value.templateDigest, `${path}.templateDigest`)
...
const assertReviewBinding = (value: unknown, path: string) => {
  assertRecord(value, path)
  assertExactKeys(value, ['schemaVersion', 'ceremonyId', 'intentRevisions', 'presentationKind', 'presentationDigest'], path)
  if (value.schemaVersion !== 1) throw new Error(`${path}.schemaVersion is unsupported`)
  assertHash(value.ceremonyId, `${path}.ceremonyId`)
...
const assertPluginEnvelope = (value: unknown, path: string) => {
  assertRecord(value, path)
...
export function assertExecutionTemplate(value: unknown): asserts value is ExecutionTemplate {
  assertRecord(value, 'template')
  assertExactKeys(value, ['schemaVersion', 'wallet', 'effects', 'transport', 'requests', 'signatureSlots', 'pythRefreshSlots', 'constraints', 'policyEvidenceDigest'], 'template')
  if (value.schemaVersion !== 1) throw new Error('template.schemaVersion is unsupported')
  assertRecord(value.wallet, 'template.wallet')
  assertExactKeys(value.wallet, ['chainId', 'account', 'subAccounts', 'connectorId', 'connectorSessionId', 'walletKind', 'safeAddress', 'classificationVersion', 'approvalMode'], 'template.wallet')
  assertSafeInteger(value.wallet.chainId, 'template.wallet.chainId', 1)
  assertAddress(value.wallet.account, 'template.wallet.account')
  assertAddressArray(value.wallet.subAccounts, 'template.wallet.subAccounts')
  assertString(value.wallet.connectorId, 'template.wallet.connectorId')
  assertString(value.wallet.connectorSessionId, 'template.wallet.connectorSessionId')
  if (value.wallet.walletKind !== 'eoa' && value.wallet.walletKind !== 'safe') throw new Error('template.wallet.walletKind is unsupported')
  if (value.wallet.safeAddress !== undefined) assertAddress(value.wallet.safeAddress, 'template.wallet.safeAddress')
  if (value.wallet.walletKind === 'safe' && value.wallet.safeAddress === undefined) throw new Error('template.wallet.safeAddress is required for Safe wallets')
  assertString(value.wallet.classificationVersion, 'template.wallet.classificationVersion')
  if (value.wallet.approvalMode !== 'permit2' && value.wallet.approvalMode !== 'approve') throw new Error('template.wallet.approvalMode is unsupported')
  if (value.transport !== 'eoa' && value.transport !== 'safe') throw new Error('template.transport is unsupported')
  const transport = value.transport
  if (value.transport !== value.wallet.walletKind) throw new Error('template transport must match the wallet kind')
  if (!Array.isArray(value.effects)) throw new Error('template.effects must be an array')
  value.effects.forEach((entry, index) => assertEffectNode(entry, `template.effects[${index}]`))
  if (!Array.isArray(value.requests)) throw new Error('template.requests must be an array')
  value.requests.forEach((entry, index) => assertRequest(entry, `template.requests[${index}]`, transport))
  if (!Array.isArray(value.signatureSlots)) throw new Error('template.signatureSlots must be an array')
  value.signatureSlots.forEach((entry, index) => assertSignatureSlot(entry, `template.signatureSlots[${index}]`))
  if (!Array.isArray(value.pythRefreshSlots)) throw new Error('template.pythRefreshSlots must be an array')
  value.pythRefreshSlots.forEach((entry, index) => assertPythSlot(entry, `template.pythRefreshSlots[${index}]`))
  if (!Array.isArray(value.constraints)) throw new Error('template.constraints must be an array')
  value.constraints.forEach((entry, index) => assertIntentConstraint(entry, `template.constraints[${index}]`))
  assertHash(value.policyEvidenceDigest, 'template.policyEvidenceDigest')
}

export function assertSealedCeremony(value: unknown): asserts value is SealedCeremony {
  assertRecord(value, 'ceremony')
  assertExactKeys(value, ['schemaVersion', 'ceremonyId', 'templateDigest', 'consentDigest', 'intents', 'template', 'policyEvidence', 'simulation', 'internalManifest', 'reviewBinding', 'validity', 'plugins'], 'ceremony')
  if (value.schemaVersion !== 1) throw new Error('ceremony.schemaVersion is unsupported')
  assertHash(value.ceremonyId, 'ceremony.ceremonyId')
  assertHash(value.templateDigest, 'ceremony.templateDigest')
  assertHash(value.consentDigest, 'ceremony.consentDigest')
  if (!Array.isArray(value.intents) || value.intents.length === 0) throw new Error('ceremony.intents must be non-empty')
  value.intents.forEach(assertOperationIntent)
  assertExecutionTemplate(value.template)
  assertPolicyEvidence(value.policyEvidence, 'ceremony.policyEvidence')
  assertSimulation(value.simulation, 'ceremony.simulation')
  assertManifest(value.internalManifest, 'ceremony.internalManifest')
  assertReviewBinding(value.reviewBinding, 'ceremony.reviewBinding')
  assertValidity(value.validity, 'ceremony.validity')
  assertPluginEnvelope(value.plugins, 'ceremony.plugins')
}
