...
import { normalizeCeremonySwapQuote, type CeremonySwapQuote } from './swap-quote'
...
const normalizeArgValue = (key: string, value: unknown, path: string): CanonicalValue => {
  if (SWAP_QUOTE_KEYS.has(key)) return normalizeCeremonySwapQuote(value as SwapQuote) as unknown as CanonicalValue
...
const quoteFrom = (args: Readonly<Record<string, CanonicalValue>>): CeremonySwapQuote | undefined => {
  const value = args.swapQuote
  return value && typeof value === 'object' && !Array.isArray(value)
    ? value as unknown as CeremonySwapQuote
    : undefined
}
...
export const inferIntentConstraints = (
  ...
  throw new Error(`Planner ${planner} requires explicit ceremony constraints`)
}
