...
export interface CeremonySwapToken {
  address: Address
  chainId: number
  decimals: number
  logoURI: string
...
export interface CeremonySwapQuote {
  schemaVersion: 1
  amountIn: bigint
  amountInMax: bigint
  amountOut: bigint
  ...
  vaultIn: Address
  receiver: Address
  tokenIn: CeremonySwapToken
  tokenOut: CeremonySwapToken
  slippageBps: number
  swap: {
...
const token = (value: SwapQuote['tokenIn']): CeremonySwapToken => ({
...
/** Converts the API/SDK quote into the strict, integer-only ceremony DTO. */
export const normalizeCeremonySwapQuote = (quote: SwapQuote): Readonly<CeremonySwapQuote> => {
  ...
  }) as Readonly<CeremonySwapQuote>
}

/** Rehydrates only the public SDK fields after the DTO has passed its schema. */
export const rehydrateCeremonySwapQuote = (quote: CeremonySwapQuote): SwapQuote => ({
