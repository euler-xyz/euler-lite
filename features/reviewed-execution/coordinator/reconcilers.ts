...
import type { ReviewedExecution, EoaRequest, SafeCall } from '../domain/reviewed-execution'
import type { SubmissionJournal } from '../persistence/journal'
...
const dispatchEvidence = async (journal: SubmissionJournal, attemptId: string): Promise<DispatchEvidence[]> => {
...
const externalEvidence = async (journal: SubmissionJournal, attemptId: string) => {
  const events = await journal.listAttemptEvents(attemptId)
  return events.flatMap((event) => {
...
export class EoaAttemptReconciler implements AttemptReconciler {
  constructor(private readonly journal: SubmissionJournal, private readonly client: EoaReconciliationClient) {}

  async reconcile(execution: ReviewedExecution, attempt: SubmissionAttempt): Promise<ReconciliationResult> {
    if (execution.requestSet.transport !== 'eoa') throw new Error('EOA reconciler received a Safe reviewed execution')
    const requests = execution.requestSet.requests as readonly EoaRequest[]
...
export class SafeAttemptReconciler implements AttemptReconciler {
  constructor(private readonly journal: SubmissionJournal, private readonly client: SafeReconciliationClient) {}

  async reconcile(execution: ReviewedExecution, attempt: SubmissionAttempt): Promise<ReconciliationResult> {
    if (execution.requestSet.transport !== 'safe') throw new Error('Safe reconciler received an EOA reviewed execution')
    const calls = execution.requestSet.requests as readonly SafeCall[]
    const artifacts = await externalEvidence(this.journal, attempt.attemptId)
