...
import type { EoaRequest, ReviewedRequestSet, FinalizedRequestSet, SafeCall } from '../domain/reviewed-execution'
...
export const finalizeReviewedRequestSet = ({
  reviewId,
  requestDigest,
  requestSet,
  sdk,
...
}): Readonly<FinalizedRequestSet> => {
  assertExactSlotSet(requestSet.signatureSlots.map(slot => slot.slotId), signatures.map(value => value.slotId), 'Signature')
  assertExactSlotSet(requestSet.pythRefreshSlots.map(slot => slot.slotId), pythValues.map(value => value.slotId), 'Pyth')

  const requests = requestSet.requests.map((sealedRequest): EoaRequest | SafeCall => {
    const requestId = requestIdOf(sealedRequest)
    const signatureSlots = requestSet.signatureSlots.filter(slot => slot.insertionPoints.some(point => point.requestId === requestId))
    const pythSlots = requestSet.pythRefreshSlots.filter(slot => slot.insertionPoint.requestId === requestId)
  ...
  assertFinalizedRequestsMatch(requestSet, requests, sdk, { signatures, pythValues })

  const artifact: FinalizedRequestSet = {
  ...
  return deepFreezeSerializable(artifact) as Readonly<FinalizedRequestSet>
}

export const assertFinalizedRequestsMatch = (
  requestSet: ReviewedRequestSet,
  ...
  if (requestSet.requests.length !== finalizedRequests.length) throw new Error('Finalized request count changed')

  const normalized = finalizedRequests.map((actual, requestIndex): EoaRequest | SafeCall => {
    const sealed = requestSet.requests[requestIndex] as EoaRequest | SafeCall
    ...
    const dynamicIndices = new Set<number>([
      ...requestSet.signatureSlots.flatMap(slot => slot.insertionPoints.filter(point => point.requestId === requestId).map(point => point.batchItemIndex)),
      ...requestSet.pythRefreshSlots.filter(slot => slot.insertionPoint.requestId === requestId).map(slot => slot.insertionPoint.batchItemIndex),
    ])
    ...
    for (const [index, item] of actualItems.entries()) {
      ...
      const signatureSlot = requestSet.signatureSlots.find(slot => slot.insertionPoints.some(point => point.requestId === requestId && point.batchItemIndex === index))
      const pythSlot = requestSet.pythRefreshSlots.find(slot => slot.insertionPoint.requestId === requestId && slot.insertionPoint.batchItemIndex === index)
  ...
  if (canonicalDigest('normalized-request-vector-v1', toCanonicalValue(normalized)) !== canonicalDigest('normalized-request-vector-v1', toCanonicalValue(requestSet.requests))) {
