...
const classifyBatchEffect = ({
  ...
  chainId: number
  item: EVCBatchItem
  pythPreviewData?: PythPreviewData
}): { effect: TypedEffect, provenance: EffectProvenance } => {
  const selector = selectorOf(item.data)
  if (selector === PYTH_SELECTOR) {
    if (!pythPreviewData) throw new Error('Pyth update is missing sealed feed and freshness evidence')
    if (getAddress(item.targetContract) !== getAddress(pythPreviewData.target)) {
    ...
    return {
      effect: {
        ...
        data: item.data,
        selector,
        requiredFeedIds: [...pythPreviewData.requiredFeedIds].sort(),
      },
...
export const materializePreparedPlan = ({
  ...
  for (const [planItemIndex, item] of plan.entries()) {
    if (item.type === 'cowSwap') throw new Error('CoW solver-order workflows are outside the reviewed execution')
  ...
  return deepFreezeSerializable(requestSet) as Readonly<ReviewedRequestSet>
}

export const reviewedRequestDigest = (requestSet: ReviewedRequestSet): Hash =>
  canonicalDigest('reviewed-request-set-v1', toCanonicalValue(requestSet))
