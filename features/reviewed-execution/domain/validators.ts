...
import type { EoaRequest, ReviewedRequestSet, SafeCall, SignatureSlot } from './reviewed-execution'
...
const validateRequestVector = (requestSet: ReviewedRequestSet) => {
  const requests = new Map<string, EoaRequest | SafeCall>()
  const effects = new Map(requestSet.effects.map(node => [node.effectId, node]))
  ...
  for (const request of requestSet.requests) {
    ...
    const requestIdentity = {
      ...
      from: requestSet.wallet.account,
      to: request.to,
      data: request.data,
      value: request.value,
    }
    ...
    if ('requestId' in request && (request.chainId !== requestSet.wallet.chainId || request.from !== requestSet.wallet.account)) {
...
const validateDynamicSlots = (
  ...
  const permitEffects = new Map(requestSet.effects
    .filter(node => node.effect.kind === 'approval' && node.effect.mode === 'permit2')
    .map(node => [node.effectId, node]))

  for (const slot of requestSet.signatureSlots) {
    if (signatureIds.has(slot.slotId)) throw new Error(`Duplicate signature slot ID ${slot.slotId}`)
    signatureIds.add(slot.slotId)
    if (slot.chainId !== requestSet.wallet.chainId) throw new Error(`Signature slot ${slot.slotId} targets another chain`)
    if (getAddress(slot.signer) !== getAddress(requestSet.wallet.account)) throw new Error(`Signature slot ${slot.slotId} has a different signer`)
  ...
  for (const slot of requestSet.pythRefreshSlots) {
    ...
    const effect = node.effect
    if (
      slot.chainId !== requestSet.wallet.chainId
...
export const validateReviewedRequestSet = (requestSet: ReviewedRequestSet, intents: readonly OperationIntent[]) => {
  validateIntentSet(intents)
  validateEffectGraph(requestSet.effects, intents)
  if (!requestSet.effects.length || !requestSet.requests.length) throw new Error('Execution requestSet is empty')
  ...
  for (const intent of intents) {
    for (const constraint of intent.constraints) {
      if (!constraintDigests.has(constraintDigest(constraint))) {
        throw new Error(`Intent ${intent.intentId} constraint is absent from the requestSet`)
      }
    }
  }

  const pythEffects = requestSet.effects.filter(node => node.effect.kind === 'pyth-update')
  if (pythEffects.length !== requestSet.pythRefreshSlots.length) throw new Error('Every Pyth effect must have exactly one refresh slot')
  for (const slot of requestSet.pythRefreshSlots) {
