...
import type { IntentSwapQuote } from '../domain/swap-quote'
import { rehydrateIntentSwapQuote } from '../domain/swap-quote'
...
const runtimeFrom = (context: IntentCompilerContext): LiteCompilerRuntime => {
  ...
  if (!runtime.account || !runtime.sdk) throw new Error('The reviewed execution compiler runtime is incomplete')
...
const rehydrateNestedPlannerArgs = (value: unknown): Record<string, unknown> => {
  ...
  if (args.swapQuote) args.swapQuote = rehydrateIntentSwapQuote(args.swapQuote as IntentSwapQuote)
  return args
}
...
const callbackCompiler = (key: 'compileCrossProtocolMigration' | 'compileRewardClaim' | 'compileREULUnlock'): IntentCompiler => ({
  async compile(intent, context) {
    ...
    if (!callback) throw new Error(`Execution runtime callback ${key} is unavailable`)
