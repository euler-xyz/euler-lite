...
7. EOA receipt sequencing remains SDK-owned. Before `wallet_sendCalls`, the Safe adapter acquires a browser-wide Web Lock, durably reserves the exact review/account/chain context, and re-runs the full wallet and policy guard. It holds the lock across the wallet prompt until the calls ID is attached or the handoff exits, so another tab cannot clear a live hashless reservation. Browsers without cross-tab locking fail closed. The adapter stores the returned calls ID as an opaque string of at most 4096 bytes before waiting for status. It uses the ID as a transaction hash for receipt fallback only when the value has transaction-hash shape. Confirmed completion applies captured state effects even when modal/navigation effects are suppressed: exact submitted revisions are removed, newer revisions survive, and tagged external migrations schedule their refreshes. Reconciliation that proves an earlier proposal succeeded refreshes portfolio data and stops the replacement dispatch so the user must prepare and review again. Conclusive revert, failure, or cancellation clears the reservation. Unknown status retains it and blocks another proposal for that Safe context until a later submission reconciles terminal calls-status evidence. A hashless reservation can be cleared in Settings only after its live handoff lock is gone and the user confirms that Safe contains no proposal.
...
| `composables/useReviewedExecution.ts` | App integration for in-memory prepare/accept, policy revalidation, Safe reconciliation, and query invalidation |
...
| `utils/pending-safe-reviewed-submission.ts` | Durable Safe handoff reservation, calls-ID binding, and context-checked clearing |
| `utils/safe-reviewed-submission-lock.ts` | Browser-wide serialization for every Safe reservation storage mutation |
| `composables/usePendingSafeReviewedSubmission.ts` | Settings-facing inspection and confirmed-absent remediation for hashless Safe reservations |
