...
7. EOA receipt sequencing remains SDK-owned. Before `wallet_sendCalls`, the Safe adapter acquires a browser-wide Web Lock and durably reserves the exact review/account/chain context; calls-ID attachment and clearing use the same serialized storage boundary. Browsers without cross-tab locking fail closed. The adapter attaches the returned calls ID before waiting for status. Confirmed completion applies captured state effects even when modal/navigation effects are suppressed: exact submitted revisions are removed, newer revisions survive, and tagged external migrations schedule their refreshes. Conclusive success, revert, failure, or cancellation clears the reservation. Unknown status retains it and blocks another proposal for that Safe context until a later submission reconciles terminal calls-status evidence. A handoff that returned no usable calls ID can be cleared in Settings only after the user confirms that Safe contains no proposal.
