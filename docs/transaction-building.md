...
7. EOA receipt sequencing remains SDK-owned. Safe handoff continues through the current-session calls-ID status and detachment flow. Confirmed completion applies captured state effects even when modal/navigation effects are suppressed: exact submitted revisions are removed, newer revisions survive, and tagged external migrations schedule their refreshes. Unknown status is reported without persistence, reconciliation, or automatic retry.
...
Submit blockers remain the operation-eligibility gate. Wallet screening is a connect-time gate (`useAddressScreen` / `useWagmi`); reviewed execution has no `wallet-screening` policy concern and does not call `screenAddress` again. `useWagmi().address` is already undefined until a non-restricted verdict, so preparation cannot start for an unscreened EOA. Reviewed execution still requires the TOS, Keyring, labels, vault metadata, authority bindings, and acknowledgements needed by the exact reviewed calls. Derived EVC accounts are bound into the reviewed request but are not screened as separate wallets. Country, vault, asset, and swap restrictions remain in the existing server and form guards. Plugin-prefetch reuse is context-bound and does not weaken those checks. See [Address Screening](./address-screening.md).
