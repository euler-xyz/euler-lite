...
| `/api/internal/oracle-adapters` | 5 min | Per-chain `adapters/all.json` (Lite UI + SDK `fetchOracleAdapterMap`). 404 → uncached `[]` |
| `/api/internal/oracle-routers` | 5 min | Per-chain recognized EulerRouter allowlist (`routers/all.json`). Same 404 / stale behaviour |
| `/api/internal/oracle-adapter` | 5 min | Per-address `{address}.json`. No Lite UI caller; prefer the bulk route |
...
**Wallet screening**: connect-time only (`useAddressScreen` / `useWagmi` gate). `POST /api/internal/screen-address` proxies to data-v3 with `chain: "all"`, fail-closed except when both `ADDRESS_SCREENING_URI` and `ADDRESS_SCREENING_API_KEY` are unset outside production. Reviewed execution does not re-screen. VPN for the upstream payload comes from Cloudflare request headers, not the JSON body. First-party `*.euler.finance` SPAs may call this one path cross-origin. Full contract, config, and troubleshooting: [Address Screening](./address-screening.md).
