...
Two handlers front that one cache. The query-shape route in the table above is what server-side callers (`labels-helpers.ts`) use; the SDK's `eulerLabelsBaseUrl` points at `/api/internal/labels`, so the SDK's own fetcher lands on the path-shape sibling at `/api/internal/labels/{chainId}/{file}`. Both read through the cache, so a warm entry serves either shape without a second upstream fetch. The `refreshLabelFile` force-refresh primitive underneath is called directly only by the warm paths (`server/plugins/warm-cache.ts` and the vault snapshot builder) — deliberately, so each cycle rewrites the entry before it expires instead of cache-hitting a nearly-expired value. `tests/server/labels-route-cache.test.ts` locks in that split.
