...
So the 5-minute TTL is not what bounds upstream traffic on the SDK's path. For that route the warm-cache entry currently functions as a stale fallback rather than as a read-through cache, and the in-flight map is the only thing collapsing duplicate work. Warm callers (`server/plugins/warm-cache.ts` and the vault snapshot builder) call `refreshLabelFile` deliberately, so each cycle rewrites the entry instead of cache-hitting a nearly-expired value — the path-shape handler simply reuses the same primitive where a read-through would be expected. Its own header comment describes a shared read-through pipeline, which overstates what it does; treat this as a known divergence rather than intended design.
