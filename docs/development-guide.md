...
| `GET\|HEAD /api/internal/proxy/turtle/:path` | `earn.turtle.xyz/v1` (`streams/merkle_proofs` allowlist only; server attaches the `X-API-Key`; answers 503 without contacting upstream when the key is unset) | not cached (`bypassCache`), `no-store` | `TURTLE_EARN_API_KEY` (server-only, required); upstream is fixed, no URL override |
