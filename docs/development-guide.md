...
| `GET\|HEAD /api/internal/proxy/turtle/:path` | `earn.turtle.xyz/v1` (`streams/merkle_proofs` allowlist only; server attaches the `X-API-Key`; answers 503 without contacting upstream when the key is unset) | not cached (`bypassCache`), `no-store` | `TURTLE_EARN_API_KEY` (server-only, required), `TURTLE_EARN_API_URL`, `NUXT_PUBLIC_TURTLE_EARN_API_URL` (`https://` on `turtle.xyz` hosts only) |
