...
| `GET /api/internal/oracle-adapters?chainId=X` | `{chainId}/adapters/all.json` from oracle-checks (Lite UI + SDK map) | 5 min; 404 → uncached `[]` | `NUXT_PUBLIC_CONFIG_ORACLE_CHECKS_BASE_URL` |
| `GET /api/internal/oracle-routers?chainId=X` | `{chainId}/routers/all.json` recognized EulerRouter allowlist | 5 min; 404 → uncached `[]` | `NUXT_PUBLIC_CONFIG_ORACLE_CHECKS_BASE_URL` |
| `GET /api/internal/oracle-adapter?chainId=X&address=0x...` | Per-adapter `{address}.json` from oracle-checks (no Lite UI caller) | 5 min | `NUXT_PUBLIC_CONFIG_ORACLE_CHECKS_BASE_URL` |
