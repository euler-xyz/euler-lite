...
2. **Nuxt `runtimeConfig`** (`useDeployConfig()`) — social links, feature flags, labels/oracle source URLs, and static deployment fallbacks. Set via `NUXT_PUBLIC_CONFIG_*` env vars. Includes `NUXT_PUBLIC_CONFIG_LABELS_BASE_URL`, `NUXT_PUBLIC_CONFIG_ORACLE_CHECKS_BASE_URL`, `NUXT_PUBLIC_CONFIG_EULER_CHAINS_URL`, and `NUXT_PUBLIC_CONFIG_EULER_ABIS_BASE_URL` for configuring upstream data sources (GitHub or S3/CDN). All of these are fetched through server-side proxy endpoints with 5-minute caching — see [Server-Side Data Proxies](#server-side-data-proxies) below.
...
All listed endpoints use rate limiting. Local-TTL endpoints (everything in the table except `/api/internal/v3/...` and `/api/internal/pyth/updates`) return stale cached data when upstream is unavailable. The euler-chains and ABI manifests keep a 7-day stale window (vs the default 30-minute ceiling) so a running instance outlives any realistic upstream outage; an instance cold-started mid-outage 502s until upstream recovers or the explicit URL env vars repoint it at a mirror. Unusable 200 payloads (empty/sparse EulerChains entries for **enabled** chains, ABI fragments missing required signatures or `outputs`) are rejected and do **not** overwrite last-known-good — see [Deployment Manifest and Runtime ABIs](./server-side-caching.md#deployment-manifest-and-runtime-abis). The V3 proxy has no local TTL cache: it forwards the upstream response, or returns `503` during failure backoff / upstream outage with `retry-after`. `/api/internal/pyth/updates` requires real-time data and returns no-store cache headers. The shared caching utility is in `server/utils/cache.ts`; the per-host external proxies share `server/utils/external-proxy.ts`. See [Server-Side Caching](./server-side-caching.md) for V3 allowlist, backoff, labels handler divergence, and manifest admission detail.

**Startup cache warming**: `server/plugins/warm-cache.ts` runs at Nitro startup. Two timers:

- **Global cycle (5 min)**: `/api/internal/euler-chains` once, the runtime ABI manifests once, cross-chain `labels/all/assets.json` once, then per-chain `/api/internal/labels/*` and `/api/internal/token-list` for every enabled non-deprecated chain, serialized so cross-chain upstreams dedupe via the in-flight cache.
...
- If `/api/internal/euler-chains` or `/api/internal/abis/{contract}` returns 502 while GitHub is 200, admission rejected the payload (enabled chain missing required core/lens addresses, or ABI missing a required signature / non-empty `outputs`). A running instance keeps last-known-good; a cold boot has nothing to serve — repoint with `NUXT_PUBLIC_CONFIG_EULER_CHAINS_URL` / `NUXT_PUBLIC_CONFIG_EULER_ABIS_BASE_URL`. See [manifest admission](./server-side-caching.md#troubleshooting).
- After adding `RPC_URL_<chainId>`, restart the dev server. That chain must already be in `entities/chainRegistry` **and** have a complete EulerChains entry, or the next euler-chains refresh throws instead of caching.
