...
| After a successful tx | `invalidateSdkQueries` marks the `invalidateAfterTx` rows stale. They stay in the cache — nothing is removed — but the next plan-time read re-fetches instead of reusing them. This is the only unconditional re-read in the flow. |
